...
// 🔗 Main Popup
const ResultPopup = ({onClose, showChatbotPopup, setShowChatbotPopup, setShowResultPopup, darkMode, selectedHazards = []}) => {

const allHazards = [
  { name: 'Flooding', description: 'Flooding in low-lying areas', recommendation: 'Use flood barriers and evacuate' },
  { name: 'Rainfall', description: 'Heavy rainfall expected', recommendation: 'Stay indoors, avoid flooding zones' },
  { name: 'Heat Index', description: 'High heat risk', recommendation: 'Drink water and stay cool' }
];
  ...
  <Document>
    <Page style={styles.page}>
      ...
      <View style={styles.section}>
        <Text style={{ fontSize: 16, marginBottom: 10 }}>Assessment Results</Text>
       ...
       <Text style={[styles.tableCell, styles.lastCell]}>AI Recommendation</Text>
      </View>
       ...
       <Text style={styles.tableCell}>{hazard.description}</Text>
        <Text style={[styles.tableCell, styles.lastCell]}>{hazard.recommendation}</Text>
      </View>
      ))}
      </View>
      </View>
  ...
  return (
    <div className="profile-popup-overlay">
      <div className={`profile-popup ${darkMode ? "dark-mode" : ""}`}>
        ...
        >
        {loading ? 'Generating PDF...' : 'View Result with AI Recommendation (PDF)'}
        </button>
        )}
        </PDFDownloadLink>
        </div>
